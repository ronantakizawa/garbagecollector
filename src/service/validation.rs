...
use crate::proto::{CreateLeaseRequest, RenewLeaseRequest, ReleaseLeaseRequest};
...
impl<'a> RequestValidator<'a> {
    ...
    pub fn validate_renew_lease_request(&self, request: &RenewLeaseRequest) -> Result<()> {
        if request.lease_id.is_empty() {
            self.metrics.record_startup_error("lease_validation", "empty_lease_id");
            return Err(GCError::Configuration("Lease ID cannot be empty".to_string()));
        }
        ...
        if request.extend_duration_seconds > 0 {
            ...
            if duration > self.config.gc.max_lease_duration_seconds {
                self.metrics.record_startup_error("lease_validation", "invalid_extension_duration");
                return Err(GCError::InvalidLeaseDuration {
                    duration,
                    min: self.config.gc.min_lease_duration_seconds,
                    max: self.config.gc.max_lease_duration_seconds,
    ...
    pub fn validate_service_id(&self, service_id: &str) -> Result<()> {
        ...
        if service_id.len() > 255 {
            return Err(GCError::Configuration("Service ID too long (max 255 characters)".to_string()));
        }
        
        // Check for valid characters (alphanumeric, hyphens, underscores)
        if !service_id.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
            return Err(GCError::Configuration(
                "Service ID can only contain alphanumeric characters, hyphens, and underscores".to_string()
            ));
        }
    ...
    pub fn validate_object_id(&self, object_id: &str) -> Result<()> {
        ...
        if object_id.len() > 1024 {
            return Err(GCError::Configuration("Object ID too long (max 1024 characters)".to_string()));
        }
    ...
    pub fn validate_lease_id(&self, lease_id: &str) -> Result<()> {
        ...
        if uuid::Uuid::parse_str(lease_id).is_err() {
            return Err(GCError::Configuration("Invalid lease ID format (must be UUID)".to_string()));
        }
    ...
    pub fn validate_metadata(&self, metadata: &std::collections::HashMap<String, String>) -> Result<()> {
        // Check metadata size limits
        if metadata.len() > 50 {
            return Err(GCError::Configuration("Too many metadata entries (max 50)".to_string()));
        }
        
        for (key, value) in metadata {
            // Validate key constraints
            if key.is_empty() {
                return Err(GCError::Configuration("Metadata key cannot be empty".to_string()));
            }
            
            if key.len() > 255 {
                return Err(GCError::Configuration("Metadata key too long (max 255 characters)".to_string()));
            }
            
            // Validate value constraints
            if value.len() > 1024 {
                return Err(GCError::Configuration("Metadata value too long (max 1024 characters)".to_string()));
            }
        }
    ...
    pub fn validate_cleanup_config(&self, config: &crate::proto::CleanupConfig) -> Result<()> {
        ...
        if !config.cleanup_http_endpoint.is_empty() {
            if let Err(_) = url::Url::parse(&config.cleanup_http_endpoint) {
                return Err(GCError::Configuration(
