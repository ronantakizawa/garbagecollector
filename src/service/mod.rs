...
use crate::metrics::{AlertThresholds, Metrics};
...
impl GCService {
    ...
    pub async fn new(config: Config) -> Result<Self> {
        ...
        if !validation_result.success {
            ...
            let error_type = validation_result
                .errors
                .first()
        ...
        info!(
            "✅ Configuration validation passed in {:.3}s",
            config_validation_duration.as_secs_f64()
        );
        ...
        let storage = match create_storage(&config).await {
            Ok(storage) => {
                ...
                metrics
                    .record_component_initialization("storage_backend", storage_creation_duration);
                info!(
                    "✅ Storage backend '{}' initialized in {:.3}s",
                    config.storage.backend,
                    storage_creation_duration.as_secs_f64()
                );
                storage
            }
            Err(e) => {
                ...
                error!(
                    "❌ Failed to create storage backend after {:.3}s: {}",
                    storage_creation_duration.as_secs_f64(),
                    e
                );
                return Err(e);
        ...
        info!(
            "✅ GarbageTruck service components initialized in {:.3}s",
        ...
        info!(
            "   • Config validation: {:.3}s",
            config_validation_duration.as_secs_f64()
        );
        info!(
            "   • Metrics system: {:.3}s",
            metrics_creation_duration.as_secs_f64()
        );
        info!(
            "   • Storage backend: {:.3}s",
            storage_creation_start.elapsed().as_secs_f64()
        );
        info!(
            "   • Cleanup executor: {:.3}s",
            cleanup_creation_duration.as_secs_f64()
        );
        ...
        info!(
            "   • Storage: {} (DB: {})",
            config_summary.storage_backend, config_summary.has_database_url
        );
        info!(
            "   • Lease duration: {}s (default)",
            config_summary.default_lease_duration
        );
        info!(
            "   • Cleanup interval: {}s",
            config_summary.cleanup_interval
        );
        info!(
            "   • Max leases/service: {}",
            config_summary.max_leases_per_service
        );
        info!(
            "   • Metrics: {} (port: {:?})",
            config_summary.metrics_enabled, config_summary.metrics_port
        );
    ...
    pub(crate) async fn run_cleanup_cycle(&self, grace_period: Duration) -> Result<usize> {
        ...
        let expired_leases = match self.storage.get_expired_leases(grace_period).await {
            Ok(leases) => {
                self.metrics
                    .record_storage_operation("get_expired_leases", &self.config.storage.backend);
                leases
            }
            Err(e) => {
                self.metrics.record_storage_error(
