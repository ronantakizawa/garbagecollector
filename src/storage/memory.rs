...
impl MemoryStorage {
    ...
    pub fn get_internal_stats(&self) -> HashMap<String, u64> {
        ...
        stats.insert(
            ...
            self.stats.operations_total.load(Ordering::Relaxed),
        );
        stats.insert(
            "get_operations".to_string(),
            self.stats.get_operations.load(Ordering::Relaxed),
        );
        stats.insert(
            "list_operations".to_string(),
            self.stats.list_operations.load(Ordering::Relaxed),
        );
        stats.insert(
            "create_operations".to_string(),
            self.stats.create_operations.load(Ordering::Relaxed),
        );
        stats.insert(
            "update_operations".to_string(),
            self.stats.update_operations.load(Ordering::Relaxed),
        );
        stats.insert(
            ...
            .filter_map(|entry| {
                ...
                if lease.state == LeaseState::Active && lease.is_expired() {
                    ...
                    let grace_expires_at =
                        lease.expires_at + chrono::Duration::from_std(grace_period).unwrap();
...
mod tests {
    ...
    async fn test_expired_leases() {
        ...
        let expired_with_grace = storage
            .get_expired_leases(std::time::Duration::from_secs(3600))
            .await
            .unwrap();
    ...
    async fn test_count_active_leases_for_service() {
        ...
        let count = storage
            .count_active_leases_for_service("service1")
            .await
            .unwrap();
