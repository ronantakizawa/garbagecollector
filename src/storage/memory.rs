...
use super::{Storage, ExtendedStorage, StorageInfo, DetailedStorageStats};
...
impl Storage for MemoryStorage {
    async fn create_lease(&self, lease: Lease) -> Result<()> {
        ...
        if self.leases.contains_key(&lease_id) {
            return Err(GCError::Internal(format!("Lease {} already exists", lease_id)));
        }
    ...
    async fn get_stats(&self) -> Result<LeaseStats> {
        ...
        debug!("Memory storage stats: {} total, {} active, {} expired", 
    ...
    async fn cleanup(&self) -> Result<()> {
        ...
        if removed_count > 0 {
            debug!("Cleaned up {} expired leases from memory storage", removed_count);
        }
...
mod tests {
    use super::*;
    use crate::lease::{Lease, ObjectType, LeaseState, LeaseFilter};
    ...
    async fn test_count_leases() {
        ...
        // Create leases
        storage.create_lease(create_test_lease("1", "service-1")).await.unwrap();
        storage.create_lease(create_test_lease("2", "service-1")).await.unwrap();
    ...
    async fn test_get_stats() {
        ...
        // Create leases
        storage.create_lease(create_test_lease("1", "service-1")).await.unwrap();
        storage.create_lease(create_test_lease("2", "service-2")).await.unwrap();
