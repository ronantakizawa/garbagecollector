...
use garbagetruck::proto::{ReleaseLeaseRequest, RenewLeaseRequest};
...
async fn test_unauthorized_access() -> Result<()> {
    ...
    assert!(
        ...
        "Renewal should fail for unauthorized service"
    );
    assert!(
        renew_result.error_message.contains("Unauthorized"),
    ...
    assert!(
        ...
        "Release should fail for unauthorized service"
    );
...
async fn test_lease_not_found() -> Result<()> {
    ...
    assert!(
        !release_result.success,
        "Should fail for non-existent lease"
    );
