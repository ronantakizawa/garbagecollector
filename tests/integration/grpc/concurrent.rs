...
async fn test_concurrent_lease_renewals() -> Result<()> {
    ...
    assert!(successful_renewals >= 1, "At least one renewal should succeed");
    println!("✅ Concurrent renewals passed ({}/5 successful)", successful_renewals);
    Ok(())
}
...
async fn test_stress_lease_creation() -> Result<()> {
    ...
    assert!(success_rate >= 80.0, "Should have at least 80% success rate");
    assert!(duration.as_secs() < 30, "Should complete within 30 seconds");
    
    println!("✅ Stress test passed: {}/{} leases created ({:.1}%) in {:.2}s", 
             successful_creates, num_leases, success_rate, duration.as_secs_f64());
    Ok(())
}