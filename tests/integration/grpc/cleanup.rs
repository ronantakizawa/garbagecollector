...
async fn test_automatic_cleanup() -> Result<()> {
    ...
    if cleanup_occurred {
        ...
        let our_cleanup = cleanup_calls
            .iter()
            .find(|call| call.lease_id == lease_id)
            .unwrap();
...
async fn test_cleanup_retry_logic() -> Result<()> {
    ...
    let cleanup_calls = harness.cleanup_server.get_cleanup_calls().await;
    let retry_calls: Vec<_> = cleanup_calls
        .iter()
    ...
    if !retry_calls.is_empty() {
        println!(
            "✅ Cleanup retry logic working (found {} retry attempts)",
            retry_calls.len()
        );
    } else {
        println!(
            "⚠️  Retry logic test inconclusive - cleanup interval may be longer than test duration"
        );
...
async fn test_cleanup_with_delay() -> Result<()> {
    ...
    let cleanup_calls = harness.cleanup_server.get_cleanup_calls().await;
    let delayed_calls: Vec<_> = cleanup_calls
        .iter()
        .filter(|call| call.lease_id == lease_id)
        .collect();
...
async fn test_immediate_cleanup_on_release() -> Result<()> {
    ...
    assert!(
        release_response.into_inner().success,
    ...
    let cleanup_calls = harness.cleanup_server.get_cleanup_calls().await;
    let immediate_calls: Vec<_> = cleanup_calls
        .iter()
        .filter(|call| call.lease_id == lease_id)
        .collect();
