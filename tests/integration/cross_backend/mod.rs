...
async fn test_storage_backend_consistency() -> Result<()> {
    ...
    memory_storage
        .create_lease(test_lease.clone())
        .await
        .map_err(|e| anyhow::anyhow!("Memory storage error: {}", e))?;

    let memory_result = memory_storage
        .get_lease(&test_lease.lease_id)
        .await
        .map_err(|e| anyhow::anyhow!("Memory storage error: {}", e))?;
    assert!(
        memory_result.is_some(),
        "Memory storage should store and retrieve lease"
    );

    let memory_stats = memory_storage
        .get_stats()
        .await
        .map_err(|e| anyhow::anyhow!("Memory storage error: {}", e))?;
    assert_eq!(
        memory_stats.total_leases, 1,
        "Memory storage should count 1 lease"
    );
    ...
    if !skip_if_no_env("DATABASE_URL") {
        ...
        let postgres_storage = PostgresStorage::new(&database_url, 5)
            .await
            .map_err(|e| anyhow::anyhow!("PostgreSQL storage error: {}", e))?;

        let pg_test_lease =
            create_test_lease_data("pg-consistency-test", "pg-consistency-service", 300);
        postgres_storage
            .create_lease(pg_test_lease.clone())
            .await
            .map_err(|e| anyhow::anyhow!("PostgreSQL storage error: {}", e))?;

        let postgres_result = postgres_storage
            .get_lease(&pg_test_lease.lease_id)
            .await
            .map_err(|e| anyhow::anyhow!("PostgreSQL storage error: {}", e))?;
        assert!(
            postgres_result.is_some(),
            "PostgreSQL storage should store and retrieve lease"
        );

        let postgres_stats = postgres_storage
            .get_stats()
            .await
            .map_err(|e| anyhow::anyhow!("PostgreSQL storage error: {}", e))?;
        assert!(
            postgres_stats.total_leases >= 1,
            "PostgreSQL storage should count leases"
        );
