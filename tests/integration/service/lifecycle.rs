...
use garbagetruck::shutdown::{TaskType, TaskPriority, ShutdownReason};
...
async fn test_service_startup_and_shutdown_cycle() -> Result<()> {
    ...
    let gc_service = create_test_service().await?;
    let coordinator = create_test_shutdown_coordinator();
    
    // Register multiple service components
    let components = [
        ("startup_validator", TaskType::Custom("startup".to_string()), TaskPriority::Critical),
        ("config_loader", TaskType::Custom("config".to_string()), TaskPriority::High),
        ("dependency_checker", TaskType::Custom("deps".to_string()), TaskPriority::High),
        ("service_monitor", TaskType::SystemMonitor, TaskPriority::Normal),
    ];
...
async fn test_service_restart_scenario() -> Result<()> {
    ...
    coordinator.update_task_handle("restartable-service", task).await;
