...
async fn test_shutdown_priority_ordering() -> Result<()> {
    ...
    let critical_handle = coordinator
        .register_task(
...
async fn test_multiple_shutdown_signals() -> Result<()> {
    ...
    coordinator
        .update_task_handle("multi-signal-task", task)
        .await;
...
async fn test_shutdown_with_mixed_task_behavior() -> Result<()> {
    ...
    let _stubborn_handle = coordinator
        .register_task(
            "stubborn-task".to_string(),
            TaskType::Custom("stubborn".to_string()),
            TaskPriority::Low,
    ...
    coordinator
        .update_task_handle("stubborn-task", stubborn_task)
        .await;
...
async fn test_shutdown_statistics_accuracy() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        coordinator
            .update_task_handle(&format!("stats-task-{}", i), task)
            .await;
    }
