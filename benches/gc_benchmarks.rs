...
async fn setup_client() -> Result<DistributedGcServiceClient<tonic::transport::Channel>, Box<dyn std::error::Error + Send + Sync>> {
    let client = DistributedGcServiceClient::connect("http://localhost:50051").await?;
    Ok(client)
}

async fn create_lease_benchmark(client: &mut DistributedGcServiceClient<tonic::transport::Channel>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
async fn list_leases_benchmark(client: &mut DistributedGcServiceClient<tonic::transport::Channel>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
fn bench_concurrent_operations(c: &mut Criterion) {
    ...
    let service_available = rt.block_on(async {
        setup_client().await.is_ok()
    });
                    ...
                    rt.block_on(async move {
                        ...
                        for i in 0..concurrency {
                            ...
                            let handle = tokio::spawn({
                                let object_id = format!("concurrent-bench-{}-{}", i, Uuid::new_v4());
                                async move {
                                    if let Ok(mut client) = setup_client().await {
                                        let request = CreateLeaseRequest {
                                            object_id,
    ...
    fn test_performance_baseline() {
        ...
        let result = rt.block_on(async {
            if let Ok(mut client) = setup_client().await {
                ...
                assert!(ops_per_sec > 10.0, "Performance below acceptable threshold: {:.2} ops/sec", ops_per_sec);
